...
void SwapChain::CreateSwapChain(const WindowInfo& info, ComPtr<ID3D12Device> device, ComPtr<IDXGIFactory> dxgi, ComPtr<ID3D12CommandQueue> cmdQueue)
{
	...
	sd.BufferCount = ECoreSetting::SWAP_CHAIN_BUFFER_COUNT;								// 전면+후면 버퍼 각각 번갈아 작업해야함
	...
	for (int32 i = 0; i < ECoreSetting::SWAP_CHAIN_BUFFER_COUNT; i++)
...
void SwapChain::CreateRTV(ComPtr<ID3D12Device> device)
{
	...
	rtvDesc.NumDescriptors = ECoreSetting::SWAP_CHAIN_BUFFER_COUNT; // SwapChain class 선언부에 그 값이다.
	...
	for(int i =0; i < ECoreSetting::SWAP_CHAIN_BUFFER_COUNT; i++)
	{
...
void SwapChain::SwapIndex()
{
	...
	_backBufferIndex = (_backBufferIndex + 1) % ECoreSetting::SWAP_CHAIN_BUFFER_COUNT;
}
